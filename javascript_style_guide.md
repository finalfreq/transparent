...
+ Use a single `var` declaration for each assignment and alphabetize variables. These variables should be desciptive enough to understand what it is for.
...
+ Skip trailing commas for single line calls.
...
+ Skip leading commas for multi-line attributes.
...
+ Make sure arrow functions are done on multiple lines. Use fat arrow instead of that, self, and whatever else people do.
...
+ Use camelCase instead of snake_case for variable names, functions, etc.

```javascript
var manuSuperVariable;

function manuSuperFunctionThatDoesNothing() {
  // Nothing!!
}
```
